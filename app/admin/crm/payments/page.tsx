  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-4">
            <h1 className="text-xl font-bold text-gray-900">LeadFlow Payments</h1>
          </div>
          <div className="relative w-full sm:max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#030256] focus:border-[#030256]"
            />
          </div>
        </div>
      </div>
      ...
      <div className="p-6 flex flex-col lg:flex-row gap-6">
        ...
        <div className={`flex-1 ${selectedRecord ? 'lg:max-w-[calc(100%-380px)]' : ''}`}>
          {/* Summary cards */}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900 mt-1">
                    ${pendingInstallments.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                  </p>
                      ...
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-gray-500">ID: #{record.displayId}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-4 px-6 font-semibold text-gray-900">
                        ${record.totalCourseFee.toLocaleString('en-US', { minimumFractionDigits: 2 })}
                      </td>
                      <td className="py-4 px-6 text-gray-700">{record.paymentMethod}</td>
                      <td className="py-4 px-6">
                        <span
        ...
        {selectedRecord && (
          <div className="w-full lg:w-[360px] shrink-0">
            <InstallmentSidebar
              ...
              onMarkAsPaid={(inst) => setRecordPaymentContext({ record: selectedRecord, installment: inst })}
            />
          </div>
        )}
      </div>
    ...
    <div className="bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden sticky top-6">
      <div className="p-4 border-b border-gray-200 flex items-start justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 mt-0.5">Student: {record.studentName}</p>
        </div>
        <button
          type="button"
          onClick={onClose}
      ...
      <div className="p-4 space-y-3 max-h-[420px] overflow-y-auto">
        {record.installments.map((inst) => (
          <InstallmentCard
            key={inst.index}
            installment={inst}
            onMarkAsPaid={inst.status === 'DUE' ? () => onMarkAsPaid(inst) : undefined}
          />
        ))}
      </div>
      <div className="p-4 border-t border-gray-200">
        <button
          type="button"
          className="w-full py-2.5 border-2 border-[#030256] text-[#030256] font-semibold rounded-lg hover:bg-[#030256] hover:text-white transition-colors"
        >
          Edit Payment Plan
        </button>
      </div>
...
function InstallmentCard({
  ...
  const border = isPaid ? 'border-green-200' : isDue ? 'border-amber-200' : 'border-gray-200';
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          {isPaid && installment.paidDate && (
            <p className="text-sm text-gray-600 mt-1">Paid on {installment.paidDate}</p>
          )}
          {isDue && installment.dueDate && (
            <p className="text-sm text-gray-600 mt-1">Due Date: {installment.dueDate}</p>
          )}
          ...
          {isDue && (
            <>
              ...
              <button
                ...
                className="px-3 py-1.5 bg-[#030256] text-white text-xs font-semibold rounded-lg hover:bg-[#04036a]"
              >
                Mark as Paid
              </button>
            </>
